  ...
  return (
    <div className=' bg-[#171717] h-screen overflow-scroll min-h-screen flex relative flex-col'>
        <div className='flex flex-col'>
            ...
            {loading ? (
                <div className='flex flex-col h-[75vh] items-center justify-center flex-grow gap-2 m-4 '>
                <Lottie className='scale-[30%] animate-pulse' animationData={loaderAnimation} loop={true} />
                </div>
            ) : (
                  ...
                  {enrolledCourses && enrolledCourses.length > 0 && enrolledCourses.map((sub,idx)=>{
                     ...
                     if(hours==2 || hours==0){
                         return (
                             ...
                             <div className={attendedToday?'relative flex ease-in flex-col gap-2 transition-all duration-150  rounded-md shadow-md bg-gradient-to-r from-lime-400 from-10%  to-emerald-500 to-50%':'relative flex flex-col gap-2 transition-all duration-150 rounded-md shadow-md bg-gradient-to-r from-lime-200 from-10% via-red-400 via-20% to-red-500 to-50%'}>
                             <div className='z-10 flex flex-col justify-start p-2'>
                                 <h1 className='font-bold text-shadow '>{timestampToTimeOfDay(sub.attendance.dates[0].time.seconds)}</h1>
                                 <h1 className='text-[1.6rem] font-bold'>{sub.course_code}</h1>
                                 <h1 className='mb-[2rem] text-[3.8rem] leading-none font-semibold'>{sub.course_name}</h1>
                             </div>
                             <div className='absolute bottom-[-1rem] right-0'>
                                 <h1 className='text-white/[70%] text-[9rem] text-shadow      leading-none'>{parseFloat(((attended/sub.completed_hours)*100).toFixed(1))}</h1>
                             </div>
                             </div>
                         ...
                         return(
                             <Link key={sub.course_code} href={`/attendance/${sub.course_code}`}>
                             <div  className={!attendedToday ? 'relative min-h-[6rem]  flex flex-col gap-2  rounded-xl shadow-md  bg-gradient-to-r from-red-900 from-10%  to-red-500 to-90%': 'relative min-h-[6rem]  flex flex-col gap-2  rounded-xl shadow-md  bg-gradient-to-r from-lime-500 from-10%  to-green-500 to-90%'}>
                             <div className='z-10 flex flex-col justify-start p-2'>
                                 <h1 className='font-semibold text-shadow '>{timestampToTimeOfDay(sub.attendance.dates[0].time.seconds)} {sub.course_code}</h1>
                                 <h1 className='font-semibold text-[3.8rem] leading-none '>{sub.course_name}</h1>
                                
                             </div>
                             <div className='absolute bottom-[-0.5rem] right-0'>
                                 <h1 className='text-white/[70%] text-[6rem] text-shadow    leading-none'>{parseFloat(((attended/sub.completed_hours)*100).toFixed(1))}</h1>
                             </div>
