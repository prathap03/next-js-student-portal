...
export default function Academic() {
  ...
  const deleteCertificate = async (name, id) => {
    await deleteDoc(doc(fireStore, `students/${user.uid}/certificates`, id)).then(() => {
      const desertRef = ref(storage, `certificates/${user.uid}/academic/${name}`);
      closeModal()
      // Delete the file
  ...
  const handleChange = (event) => {
    ...
    const q = query(collection(fireStore, `students/${user.uid}/certificates`), search ? (where('name', '>=', search), where('name', '<=', search + '~'), where("type", "==", "academic")) : where("type", "==", "academic"));
  ...
  useEffect(() => {
    ...
    const q = query(collection(fireStore, `students/${user.uid}/certificates`), where("type", "==", "academic"));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const cities = [];
      querySnapshot.forEach((doc) => {
        cities.push(doc.data());
    ...
    <div className='flex flex-col justify-center flex-shrink gap-10 p-10 mini:p-2 pl-20 xl:w-[80%]  md:w-max sm:w-max '>
          ...
          <div className='flex flex-col w-[90vw] md:w-[73vw] h-[68vh] m-4 gap-2 flex-grow  rounded-lg shadow-xl   bg-[#F8F5F5]'>
            <div className='flex justify-between mt-4 ml-4 mr-4'>
              <h1 className='text-[1rem] font-semibold'>Academic</h1>
