...
export default function Workshops() {
  ...
  const deleteCertificate = async (name, id) => {
    await deleteDoc(doc(fireStore, `students/${user.uid}/certificates`, id)).then(() => {
      const desertRef = ref(storage, `certificates/${user.uid}/Workshop or skill/${name}`);
      closeModal()
      // Delete the file
  ...
  const handleChange = (event) => {
    ...
    const q = query(collection(fireStore, `students/${user.uid}/certificates`), value ? where('name', '<=', value + '~') : where("type", "==", "Workshop or skill"));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const certificateData = [];
      querySnapshot.forEach((doc) => {
        if (doc.data().name.toLowerCase().includes(value.toLowerCase())) {
          certificateData.push(doc.data());
        }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      console.log("cert: ", typeof (certificateData))
        ...
        <div className='flex h-[calc(100vh_-_81px)]'>
          ...
          <div className='flex flex-col gap-8 p-8'>
            ...
            <div className='flex flex-col w-[93rem] gap-2 flex-grow  rounded-lg shadow-xl  bg-[#F8F5F5]'>
              ...
              <div className='flex flex-wrap flex-grow gap-10 p-4 m-2 transition duration-150 ease-in'>
                ...
                {Data && Data.length !== 0 ? (
                  Data.map((data) => {
                    return (
                      ...
                      <div className='transition duration-150 ease-in' key={data.name} onClick={() => { openModal(data.imgUrl, data) }}>
